export function getRandomScheduleWithScore(input) {
    const schedule = getRandomSchedule(input);
    schedule.score = scoreSchedule(schedule, input);
    return schedule;
}
export function getCodesAtTime(schedule, day, period) {
...
export function getScoreScheduleFunction(input) {
...
function scoreSchedule(schedule, input) {
    const score = schedule.data
        .map(l => schedule.data.filter(l2 => l2.id !== l.id &&
        l2.period === l.period &&
        l2.day === l.day &&
...
export function crossoverSchedules(scheduleA, scheduleB) {
    const newSchedule = { score: -1, data: [] };
    for (let idCounter = 0; idCounter < scheduleA.data.length; idCounter++) {
        if (Math.random() < 0.5)
            newSchedule.data.push(scheduleA.data.find((lesson) => lesson.id == idCounter));
        else
            newSchedule.data.push(scheduleB.data.find((lesson) => lesson.id == idCounter));
    }
    return newSchedule;
}
export function getSchedulesDiversity(schedules) {
        ...
        for (let scheduleB of schedules) {
            ...
            for (let lessonA of scheduleA.data) {
                ...
                if (lessonA.day != lessonB.day)
                    difference++;
                if (lessonA.period != lessonB.period)
                    difference++;
                if (lessonA.room != lessonB.room)
                    difference++;
            }
        }
    }
...
export function getMutateScheduleFunction(input) {
    return function (schedule) {
        return mutateSchedule(schedule, input);
    };
}
function mutateSchedule(schedule, input) {
    const newSchedule = { score: -1, data: [] };
    const mutatedLesson = { ...getRandomItemFromList(schedule.data) };
